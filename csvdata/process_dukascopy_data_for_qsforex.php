...
/*
    ...
    Modified slightly by Angus North <angusjnorth@gmail.com> to suit
    Michael Halls-Moore's QSForex project at 
    https://github.com/mhallsmoore/qsforex. 
    Daily files named with dates are produced instead of a large
    single output file.
*/
if ($argc != 4) {
...
if (strlen($argv[2]) != 8 || strlen($argv[3]) != 8) {
    echo "The input dates must be of the form YYYYMM, example 20080801\n";
    exit(1);
}
$extract = '';
$iswindows = false;
...
if ($startyear < 1900 || $startyear > 2100 || $endyear < 1900 || $endyear > 2100 || $startmonth < 1 || $startmonth > 12 || $endmonth < 1 || $endmonth > 12 || $startday < 1 || $startday > 31 || $endday < 1 || $endday > 31) {
    echo "The input dates must be of the form YYYYMMDD, example 20080801\n";
    exit(1);
}

$starttime = gmmktime(0,0,0,$startmonth,$startday,$startyear);
$endtime = gmmktime(0,0,0,$endmonth,$endday+1,$endyear);
...
for($i = $starttime; $i < $endtime; $i += 3600) {
    ...
    $monthqsf = str_pad(gmstrftime('%m',$i), 2, '0', STR_PAD_LEFT);
    $day = gmstrftime('%d',$i);
    $hour = gmstrftime('%H',$i);
